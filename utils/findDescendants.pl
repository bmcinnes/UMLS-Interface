...
findDescendants.pl - This program returns the ancestors of a given CUI based on the configuration file. 
...
Usage: findDescendants.pl [OPTIONS] CUI
...
my $term = $umls->getPreferredTerm($cui); 
print "The descendants of $term ($cui) are: \n\n";
foreach my $descendant (sort keys %{$hashref}) {
    my $t = $umls->getPreferredTerm($descendant); 
    print "  $descendant : $t\n";
}
...
sub minimalUsageNotes {
    ...
    print "Usage: findDescendants.pl [OPTIONS] CUI\n";
    &askHelp();
    exit;
}
...
##############################################################################
sub showVersion {
    print '$Id: findDescendants.pl,v 1.1 2016/10/18 16:10:06 btmcinnes Exp $';
...
##############################################################################
sub askHelp {
    print STDERR "Type findDescendants.pl --help for help.\n";
}
